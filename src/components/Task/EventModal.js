...
export function EventModal({
  ...
  const handleDelayedClose = () => {
    setTimeout(() => {
      ...
      onClose();
    }, 2000); // 0.5 second delay to show toast
  };
  ...
  const handleRemoveSubtask = async (taskId, subTaskId) => {
    setConfirmTitle("Remove Subtask")
    setConfirmMessage("Do you really want to remove this subtask?")
  ...
  const handleRemoveTask = async (taskId) => {
    setConfirmTitle("Remove Task")
    setConfirmMessage("Do you really want to remove this task?")
    setConfirmAction(() => async () => {
      try {
